...
public class WorkspaceServiceImpl implements WorkspaceService {
    ...
    public BusinessDataVO getBusinessData(LocalDateTime beginTime, LocalDateTime endTime) {
        ...
        Integer totalOrderCount = ordersList == null ? 0 : ordersList.size();
        ...
        //有效订单数
        List<Orders> validOrdersList = new ArrayList<>();
        if (ordersList != null && ordersList.size() > 0) {
            for (Orders orders : ordersList) {
                if (orders.getStatus().equals(Orders.COMPLETED)) {
                    validOrdersList.add(orders);
                }
            }
        }
        Integer validOrderCount = validOrdersList == null ? 0 : validOrdersList.size();
        ...
        if (totalOrderCount != 0 && validOrderCount != 0) {
            ...
            //平均客单价
            unitPrice = turnover.divide(BigDecimal.valueOf(validOrderCount), 2, RoundingMode.HALF_UP);
        }

        //新增用户数
        Integer newUsers = userMapper.deltaByMap(map);
